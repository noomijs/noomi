...
export class StaticResource {
     ...
     */
    static async load(request: HttpRequest, response: HttpResponse, path: string, zip?: boolean): Promise<number | WebCacheItem> {
        // 检测路径是否在static map中
        ...
        // 文件路径
        const filePath: string = Util.getAbsPath([path]);
        // 缓存数据对象
        let cacheData;
        // 状态码
        if (WebConfig.useServerCache) { //从缓存取，如果用浏览器缓存数据，则返回0，不再操作
            const ro: number | object = await WebCache.load(request, response, path);
     ...
     * @returns         cache数据对象
     */
    static async readFile(path: string, zip?: boolean): Promise<WebCacheItem> {
        const fs = App.fs;
        // 压缩数据buffer
        let zipBuf: Buffer;
        ...
        // mime 类型
        const mimeType: string = App.mime.getType(path);
        // zip文件流
        ...
        // 从源输入流读数据
        const srcBuf = <Buffer>await new Promise((res) => {
        ...
        // 不缓存数据标志
        const saveData: boolean = this.checkNeedZip(mimeType);
        if (zip && saveData) {
            // 生成临时文件
